...
// 특정 책의 전체 장르 목록 조회
export const getGenresByBookId = async (bookId: number): Promise<{ genreId: number; genreName: string }[]> => {
  const [rows] = await pool.query<RowDataPacket[]>(`
    SELECT 
  ...
  // genres 중복 제거 (optional)
  const genreMap = new Map<number, { genreId: number; genreName: string }>();
  for (const row of rows) {
    genreMap.set(row.genreId, {
      genreId: row.genreId,
