import { searchBooksFromAladin, getBookDetailFromAladin, viewBestsellersFromAladin, } from "../repositories/aladin.repository.js";
import { findBookByItemId, insertBook, findGenresByBookId, findOrCreateGenre, linkBookGenre, } from "../repositories/books.repository.js";
import HttpErrors from "http-errors";
// searchBooks(), viewBestsellers()의 공통 로직을 처리하는 내부 헬퍼 함수
const processBookSearchResponse = (aladinResponse) => {
    const items = (aladinResponse.item ?? []).map((item) => {
...
export const searchBooks = async (query, start = 1, maxResults = 30) => {
...
export const getBookDetail = async (bookId) => {
    ...
    if (existingBook) {
        ...
        let finalDateString;
        // 타입 에러 방지
        if (publishedDateValue instanceof Date) {
            finalDateString = publishedDateValue.toISOString();
        }
        else if (typeof publishedDateValue === "string") {
            finalDateString = publishedDateValue;
        }
        else {
            finalDateString = "";
        }
        bookItem = {
            itemId: parseInt(existingBook.aladin_item_id),
            title: existingBook.title,
            author: existingBook.author ?? "",
            publisher: existingBook.publisher ?? "",
            pubDate: finalDateString,
            description: existingBook.description ?? "",
            isbn13: "",
            cover: existingBook.thumbnail_url ?? "",
            categoryNames: [],
        };
    }
    else {
        ...
        // 페이지 수 추출
        page = aladinBook.subInfo?.itemPage ?? null;
        // DB에 책 저장
...
export const viewBestsellers = async (start = 1, maxResults = 30) => {
