...
import { getDiscussionsByUserId, countDiscussionsByUserId, } from "../repositories/discussions.repository.js";
import { getUserById, getExpByUserId, getPreferredGenresByUserId, getBookmarksByUserId, } from "../repositories/mypage.repository.js";
// 페이지네이션 입력값 검증 및 메타데이터 계산 헬퍼 함수
const processPagination = (page, limit, totalCount) => {
...
export const getUserBookmarks = async (userId, page, limit) => {
    ...
    const bookmarks = await Promise.all(bookmarkRows.map(async (row) => {
...
export const getMyQuotesService = async (userId, page, limit) => {
...
// 내가 작성한 토론 조회 (페이지네이션)
export const getMyDiscussionsService = async (userId, page, limit) => {
    ...
    try {
        ...
        const data = discussions.map((row) => {
            const date = new Date(row.created_at);
            const yy = String(date.getFullYear()).slice(-2);
            const mm = String(date.getMonth() + 1).padStart(2, "0");
            const dd = String(date.getDate()).padStart(2, "0");
