...
// 북마크 추가
export const addBookmark = async (userId, aladinItemId) => {
...
// 북마크 삭제
export const removeBookmark = async (userId, aladinItemId) => {
    const bookRow = await findBookByItemId(aladinItemId.toString());
    if (!bookRow) {
        throw HttpError(404, "책 정보를 찾을 수 없습니다.");
    }
...
// 온보딩 시 좋아하는 책 북마크
export const selectFavoriteBooks = async (aladinItemIds, userId) => {
    let bookmarkIds = [];
